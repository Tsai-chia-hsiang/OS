...
static inline int my_spin_lock (atomic_int *lock) {
    ...
    /*如果lock 值是0， atomic_exchange_explicit將lock換為1 後，回傳0（即lock舊值），的到  	lock，直接回傳。若lock 不是0，則不會回傳0，要進入下面while spin*/
    do {
        do {
            asm("pause"); //讓CPU負擔小一些
        } while (*lock != 0);
	...
	//最先執行到 atomic_compare_exchange_weak_explicit的process將lock 改為1 ，該函數回傳t	rue(lock == val)。之後執行到的人都回傳false而回到while繼續等待。
